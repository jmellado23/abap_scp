CLASS zcl_data_file_upload_jmf  DEFINITION
PUBLIC
FINAL
CREATE PUBLIC .
PUBLIC SECTION.
...
string.
ENDCLASS.
CLASS zcl_data_file_upload_jmf IMPLEMENTATION.
...
* extract the filename and fileext from this metadataas well as the raw file data.
...
IF
* xco_cp_abap_repository=>object->tabl->database_table->for(iv_name = CONV #(tablename ) )->exists( ) = abap_false
* OR
...
IF fileext <> `json`.
response->set_status( i_code = if_web_http_status=>bad_request i_reason = `File type not supported` ).response->set_text( `File type not supported` ).
RETURN.
ENDIF.
...
ENDIF.
TRY.
INSERT (tablename) FROM TABLE
@<table_structure>.
IF sy-subrc = 0.
response->set_status( i_code = if_web_http_status=>ok i_reason = `Table updated successfully` ). response->set_text( `Table updated successfully` ).
ENDIF.
CATCH cx_sy_open_sql_db INTO
DATA(db_exception).
response->set_status( i_code = if_web_http_status=>bad_request i_reason = db_exception->get_text( ) ). response->set_text( db_exception->get_text( ) ).
RETURN.
ENDTRY.
ENDCASE.
ENDMETHOD.
...
| input.setPlaceholder("TargetABAP Table") \n| &&
